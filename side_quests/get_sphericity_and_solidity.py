...
def main():
    ...
    for idx, fpath in enumerate(track(maps_paths)):
        ...
        pdeets["particles"][pdb_id]["Solidity"] = float(solidity)
        # pdeets[pdb_id]["MaxFeretsDiameter"] = float(max_feret_diameter)

        print(f"{idx+1}: PDB ID: {pdb_id}\tSolidity: {solidity}")
